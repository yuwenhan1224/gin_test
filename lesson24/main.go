...
func main() {
	...
	u2:=User{Name:"wangyuanyuan",Age:20,Active: false}
	db.Debug().Create(&u2)
	...
	//根据符合条件的去删除
   db.Where("name=?","yuwenhan").Delete(User{})

	//查询被软删除的字段
	var u4 [] User
	db.Debug().Unscoped().Where("name=?","yuwenhan").Find(&u4)
    fmt.Println(u4)
	//物理删除

	//物理删除
	db.Debug().Unscoped().Where("name=?","wangyuanyuan").Delete(User{})
}
